...
def get_clothing(root, cfg_trainer, num_samples=0, train=True,
    ...
    if train:
        train_dataset = Clothing(root, cfg_trainer, num_samples=num_samples,
                                 train=train, transform=transform_train)
        val_dataset = Clothing(
            root, cfg_trainer, val=train, transform=transform_val)
...
class Clothing(torch.utils.data.Dataset):
    ...
    def __init__(self, root, cfg_trainer, num_samples=0, train=False,
                 val=False, test=False, transform=None, num_class=14):
        ...
        if train:
            ...
            for id_raw, impath in train_imgs:
                ...
                if class_num[label] < (num_samples/14) and \
    ...
    def __getitem__(self, index):
        ...
        if self.test or self.val:
            ...
            return img0, target, id_raw, target, index
