    ...
    def __init__(self, data_dir, batch_size, shuffle=True,
        ...
        noise_file = '%sCIFAR10_%.1f_Asym_%s.json' %\
            (config['data_loader']['args']['data_dir'],
             cfg_trainer['percent'], cfg_trainer['asym'])

        self.train_dataset, self.val_dataset = get_cifar10(
            config['data_loader']['args']['data_dir'], cfg_trainer,
            train=training,
            transform_train=transform_train, transform_val=transform_val,
            noise_file=noise_file, validation_split=validation_split)
...
class Clothing1MDataLoader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, shuffle=True,
        ...
        self.train_dataset, self.val_dataset = get_clothing(
            config['data_loader']['args']['data_dir'], cfg_trainer,
            num_samples=self.num_batches*self.batch_size, train=training,
...
class WebvisionDataLoader(BaseDataLoader):
    def __init__(self, data_dir, batch_size, shuffle=True,
                 validation_split=0.0, num_batches=0, training=True,
                 num_workers=4, pin_memory=True, num_class=50):
        ...
        self.train_dataset, self.val_dataset = get_webvision(
            ...
            num_samples=self.num_batches*self.batch_size, train=training,
            transform_train=self.transform_train,
            transform_val=self.transform_val, num_class=num_class)
