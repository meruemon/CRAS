...
class ConfigParser:
    ...
    def __init__(self, args, options='', timestamp=True):
        ...
        if self.config['dataset'] == 'clothing1m' or \
                self.config['dataset'] == 'webvision':
            exper_name = self.config['name']
        else:
            if self.config['trainer']['asym']:
                exper_name = self.config['name'] + '_asym_' + \
                    str(int(self.config['trainer']['percent']*100))
            else:
                exper_name = self.config['name'] + '_sym_' + \
                    str(int(self.config['trainer']['percent']*100))
