...
def main(config: ConfigParser):
    ...
    if config['method'] == "proposed":
        loader = getattr(module_data, config['data_loader']['type'])(
            config['data_loader']['args']['data_dir'],
            batch_size=5000,
            shuffle=False,
    ...
    if hasattr(data_loader1.dataset, 'num_raw_example') and \
            hasattr(data_loader2.dataset, 'num_raw_example'):
    ...
    optimizer1 = config.initialize(
        'optimizer1', torch.optim, [{'params': trainable_params1}])
    optimizer2 = config.initialize(
        'optimizer2', torch.optim, [{'params': trainable_params2}])

    lr_scheduler1 = config.initialize(
        'lr_scheduler', torch.optim.lr_scheduler, optimizer1)
    lr_scheduler2 = config.initialize(
        'lr_scheduler', torch.optim.lr_scheduler, optimizer2)

    trainer = Trainer(model1, model2, model_ema1, model_ema2,
                      train_loss1, train_loss2, metrics,
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--beta', '--beta'], type=float,
                   target=('train_loss', 'args', 'beta')),
        CustomArgs(['--lambda', '--lambda'], type=float,
                   target=('train_loss', 'args', 'lambda')),
        CustomArgs(['--percent', '--percent'], type=float,
                   target=('trainer', 'percent')),
        CustomArgs(['--asym', '--asym'], type=bool,
                   target=('trainer', 'asym')),
        CustomArgs(['--name', '--exp_name'], type=str, target=('name',)),
        CustomArgs(['--malpha', '--mixup_alpha'], type=float,
                   target=('mixup_alpha',)),
        CustomArgs(['--ealpha', '--ema_alpha'], type=float,
                   target=('ema_alpha',)),
        CustomArgs(['--nb', '--num_batches'], type=float,
                   target=('data_loader', 'args', 'num_batches')),
        CustomArgs(['--warm', '--warmup'], type=int,
                   target=('trainer', 'warmup')),
        CustomArgs(['--seed', '--seed'], type=int, target=('seed',)),
        CustomArgs(['--wc1', '--weight_decay1'], type=float,
                   target=('optimizer1', 'weight_decay')),
        CustomArgs(['--wc2', '--weight_decay2'], type=float,
                   target=('optimizer2', 'weight_decay')),
        CustomArgs(['--estep', '--ema_step'], type=float,
                   target=('ema_step',)),
        CustomArgs(['--method', '--method'], type=str,
                   target=('method',)),
        CustomArgs(['--s', '--save_dir'], type=str,
                   target=('trainer', 'save_dir')),
        CustomArgs(['-t', '--p_cutoff'], type=float,
                   target=('train_loss', 'args', 'p_cutoff')),
        CustomArgs(['--num_workers', '--num_workers'], type=int,
                   target=('data_loader', 'args', 'num_workers')),
        CustomArgs(['--epochs', '--epochs'], type=int,
                   target=('trainer', 'epochs')),
        CustomArgs(['--lt', '--loss_type'], type=str,
                   target=('train_loss', 'type'))
    ]
