...
from utils import inf_loop, sigmoid_rampup, Get_Scalar
import warnings
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model1, model2, model_ema1, model_ema2,
                 ...
                 loader=None):
        super().__init__(model1, model2, model_ema1, model_ema2,
                         train_criterion1, train_criterion2, metrics,
        ...
        self.p_fn = Get_Scalar(config['train_loss']['args']['p_cutoff'])
        ...
        for i, metric in enumerate(self.metrics):
            ...
            if self.writer is not None:
                self.writer.add_scalar(
                    '{}'.format(metric.__name__), acc_metrics[i])
        return acc_metrics

    def _train_epoch(self, epoch, model, model_ema, model_ema2, data_loader,
                     train_criterion, optimizer, lr_scheduler,
                     device='cpu', queue=None):
        """
        ...
        local_step = 0
        mask_li, select_li = [], []
        amp_cm = autocast if self.config['amp'] else contextlib.nullcontext

        selected_label = torch.ones(
            (len(data_loader.dataset),), dtype=torch.long,)*-1
        selected_label = selected_label.to(device)
        classwise_acc = torch.zeros((self.config['num_classes'],)).to(device)

        with tqdm(data_loader) as progress:
            for batch_idx, (data, target, indexs, _, idxs) \
                ...
                target = torch.zeros(
                    len(target), self.config['num_classes']
                    ).scatter_(1, target.view(-1, 1), 1)
                data = data.to(device, non_blocking=True)
                target = target.float().to(device, non_blocking=True)
                target_original = target_original.to(device, non_blocking=True)

                data, target, mixup_l, mix_index = self._mixup_data(
                    data, target, self.config['mixup_alpha'], device=device)
                ...
                train_criterion.update_hist(
                    epoch, output_original, indexs.numpy().tolist(),
                    mix_index=mix_index, mixup_l=mixup_l)

                # class condition count
                pseudo_counter = Counter(selected_label.tolist())
                if max(pseudo_counter.values()) < len(data_loader.dataset):
                    if self.config['thresh_warmup']:
                        ...
                        for i in range(self.config['num_classes']):
                            classwise_acc[i] = \
                                pseudo_counter[i]/max(wo_negative_one.values())

                local_step += 1
                with amp_cm():
                    p_cutoff = self.p_fn(self.global_step)
                    loss, _, mask, select, pseudo_lb = train_criterion(
                        self.global_step + local_step, output, target,
                        classwise_acc, p_cutoff)

                    if self.config['train_loss']['type'] == "CurriculumRegularizationLoss":
                        if idxs[select == 1].nelement() != 0:
                            selected_label[idxs[select == 1]] == \
                                pseudo_lb[select == 1]

                        mask_li.append(mask.item())
                        select_li.append(select.float().mean().item())
                ...
                self.update_ema_variables(
                    model, model_ema, self.global_step + local_step,
                ...
                total_metrics += self._eval_metrics(
                    output, target.argmax(dim=1))
                if output_original is not None:
                    total_metrics_ema += self._eval_metrics(
                        output_original, target.argmax(dim=1))
        ...
        log = {
            ...
            'mask_ratio':
                1. - sum(mask_li)/len(mask_li) if len(mask_li) != 0 else 1.,
            'select_ratio': 1. - sum(select_li)/len(select_li)
            if len(select_li) != 0 else 1.,
        }
    ...
    def _valid_epoch(self, epoch, model1, model2, device='cpu', queue=None):
        ...
        if queue is None:
            return {
                ...
                'val_metrics': (total_val_metrics / len(self.valid_data_loader)
                                ).tolist()
            }
        else:
            queue.put({
    ...
    def _warmup_epoch(self, epoch, model, data_loader, optimizer,
                      train_criterion, lr_scheduler, device='cpu', queue=None):
        total_loss = 0
        total_metrics = np.zeros(len(self.metrics))
        model.train()
        ...
        with tqdm(data_loader) as progress:
            for batch_idx, (data, target, indexs, _) in enumerate(progress):
                ...
                train_criterion.update_hist(
                    epoch, out_prob, indexs.cpu().detach().numpy().tolist())
    ...
    def update_ema_variables(self, model, model_ema, global_step,
        ...
        if alpha_ == 0:
            for ema_param, param in zip(model_ema.parameters(),
                                        model.parameters()):
                ema_param.data = param.data
        else:
            if self.config['ema_update']:
                alpha = sigmoid_rampup(
                    global_step + 1, self.config['ema_step'])*alpha_
            else:
    ...
    def _mixup_criterion(self, pred, y_a, y_b, lam, *args):
        ...
        return lam * loss_a + (1 - lam) * loss_b, \
            lam * prob_a + (1-lam)*porb_b, lam * entropy_a + (1-lam)*entropy_b
